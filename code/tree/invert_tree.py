...
class Solution:
    def main_func(self, list):
        '''
        镜面反转二叉树
        :param list:
        :return:
        '''
        tree = self.hierachicalTree(list)
        self.invertTree(tree)
        post_tree = self.inorder(tree)
        print(' '.join(tuple(post_tree)))

    def invertTree(self, tree):
        if tree: # 停止条件
            tree.leftC, tree.rightC = tree.rightC, tree.leftC # 镜面反转
            self.invertTree(tree.leftC) # 遍历左子树
            self.invertTree(tree.rightC) # 遍历右子树

    def inorder(self, tree): # 中序遍历
        if tree == None: # 停止条件，当没有左右子节点时，则返回空列表
            return []
        else:
            ...
            return left + [str(tree.data)] + right
        ...
        while i < len(list) and len(queue) > 0:  # 至少要有一个节点正在接受遍历
            ...
            if list[i] == None:  # 如果值为空，则不需要对子节点进行更新
                ...
                if current_tree.leftC_flag == 0 and current_tree.rightC_flag == 0:  # 如果当前数字值不为空，则先增加左子节点
                    ...
                    current_tree.leftC_flag = 1
                    queue.append(current_tree.leftC)  # 为了下一步再从左子节点开始
